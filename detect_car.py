...
Usage:
    $ python path/to/detect.py --weights yolov5s.pt --source 0  # webcam
     ...
     python /home/pi/yolov5/detect_car.py --weights /home/pi/yolov5/emblem6.pt --max-det 1 --nosave --conf-thres 0.4 --imgsz 256 --source 'rtsp://192.168.5.1:554/11'
"""
import os
...
def run(weights=ROOT / 'emblem4.pt',  # model.pt path(s)
    ...
    view_img = False
    start = time.time()
    drive_line_start, drive_line_stop = (0, 120), (935, 125)
    last_x = drive_line_stop[1]
    def line(x):
    ...
    def drive_sign(run, forward, side_offset):
        if run:
            if forward <= 0:
                light_out("stop")
            elif side_offset > 16:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            k = 1
            global trig, first_run, light_timeout
            while not trig:
                try:
                    state = GPIO.input(trig_pin)
                    if state == True and not light_timeout:
                        #print("run")
                        start = time.time()
                        trig = True
                        first_run = True
            ...
            drive_line = cv2.line(im0, drive_line_start, drive_line_stop, (255, 0, 0), 2)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    light_up = True
                    if center_point[0] >= (910):
                        global i_garasje
                        i_garasje = True
                        end = time.time()
                        duration = end-start
                        if first_run:
                            #print("om det ikke kjøres innen 4 min, skrus det av")
                            #print(abs(center_point[0]-last_x))
                            if abs(center_point[0]-last_x) < 12 or abs(center_point[0]-last_x) > 900: #om emblemet ikke har beveget seg mer enn 20 piksler. dvs. at det står i ro
                                light_up = False
                            if duration > 4*60:
                                light_timeout = True
                                trig = False
                            ...
                            #print("lyset skrus av om 30 sek, selv om garasjeporten er åpen")
                            if duration > 30:
                                light_timeout = True
                                trig = False
                        last_x = center_point[0]
                    else:
                        start = time.time()
                        first_run = False
            ...
            if view_img:
                ...
                dim = (width, height)
                view_im0 = cv2.resize(im0, dim, interpolation = cv2.INTER_AREA)
                cv2.imshow(str(p), view_im0)
